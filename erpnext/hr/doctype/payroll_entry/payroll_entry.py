...
from frappe.utils import cint, flt, nowdate, add_days, getdate, fmt_money, add_to_date, DATE_FORMAT,datetime
from frappe.utils import cstr, get_first_day, get_last_day, date_diff, add_days
from frappe import _
from erpnext.accounts.utils import get_fiscal_year
from erpnext.hr.doctype.attendance.attendance import get_total_hrs
...
class PayrollEntry(Document):
	...
	def get_emp_list(self):
		...
		#get_total_hrs = get_total_hrs(self)
		mydate= datetime.datetime.strptime(self.start_date, "%Y-%m-%d")
		month=mydate.month
		year=mydate.year
		overtime_hr_cost = frappe.db.get_single_value("HR Settings", "overtime_hour_price")
		##########
		...
		if sal_struct:
			...
			emp_list = frappe.db.sql("""
				select
					t1.name as employee, t1.employee_name, t1.department, t1.designation, t1.basic_salary,(t1.total_work_hrs*{total_days_in_month}) as real_attendance_hrs,
t3.total_earning as earning,t3.total_deduction as deduction,
ABS(t1.basic_salary/{total_days_in_month}/t1.total_work_hrs) as hour_cost ,

ifnull((select sum(round(TIMESTAMPDIFF(MINUTE,attendance_time,departure_time)/60,2)) as total_hours from tabAttendance where docstatus = 1 and employee=t1.name and month(attendance_date)={month} and year(attendance_date)= {year} ),0) as total_attendance_hrs ,

ifnull((select sum(round(total_hours,2)) as overtime_hours from tabTimesheet where docstatus = 1 and employee=t1.name and month(start_date)={month} and year(start_date)= {year}),0) as submitted_overtime, 

(((t1.total_work_hrs*{total_days_in_month}) - (ifnull((select sum(round(TIMESTAMPDIFF(MINUTE,attendance_time,departure_time)/60,2)) as total_hours from tabAttendance where docstatus = 1 and employee=t1.name and month(attendance_date)={month} and year(attendance_date)= {year} ),0)))* (ABS(t1.basic_salary/{total_days_in_month}/t1.total_work_hrs))) as discount_amount,

(t1.basic_salary + t3.total_earning +(ifnull((select sum(round(total_hours,2)) as overtime_hours from tabTimesheet where docstatus = 1 and employee=t1.name and month(start_date)={month} and year(start_date)= {year}),0)*{overtime_hr_cost}) - t3.total_deduction -(((t1.total_work_hrs*{total_days_in_month}) - (ifnull((select sum(round(TIMESTAMPDIFF(MINUTE,attendance_time,departure_time)/60,2)) as total_hours from tabAttendance where docstatus = 1 and employee=t1.name and month(attendance_date)={month} and year(attendance_date)= {year} ),0)))* (ABS(t1.basic_salary/{total_days_in_month}/t1.total_work_hrs)))    ) as total_after_discount
				from
					`tabEmployee` t1 left join `tabSalary Structure Employee` t2
					 on t1.name = t2.employee
					 left join `tabSalary Structure` t3
					 on t3.name = t2.parent
				where
					t1.docstatus!=2

			%s """.format(total_days_in_month=total_days_in_month,month=month,year=year,overtime_hr_cost=overtime_hr_cost)% cond, {"sal_struct": sal_struct}, as_dict=True)
			return emp_list
	...
	def get_filter_condition(self):
		...
		cond = ''
		for f in ['company', 'branch', 'department', 'designation','work_shift','gender','employment_type','employee_name']:
			if self.get(f):
				if f=='employee_name': 
					cond += " and t1.employee_number = "+self.employee_name
					continue	
...
def get_start_end_dates(payroll_frequency, start_date=None, company=None):
	...
	if payroll_frequency == "Monthly" or payroll_frequency == "Bimonthly" or payroll_frequency == "" or payroll_frequency == "Hourly":
