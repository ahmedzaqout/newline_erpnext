...
from frappe.utils import now, global_date_format, format_time,format_datetime, now_datetime
...
class Attendance(Document):
	def validate_duplicate_record(self):
		...
		if res:
			frappe.throw(_("Attendance for employee {0} is already marked").format(frappe.session.user))
...
def update_holiday():
	...
	for emp in employees:
		...
		if holiday_list:
			...
			for holiday in holidays:
				day = calendar.day_name[getdate(holiday).weekday()]
				update_att_holiday(emp,day,holiday)
...
def update_att_holiday(emp,day,holiday):
	print str(day)
	att_time, att_status, dep_time, dep_status= None, None, None, None
	employee_work_shift = frappe.db.get_value("Employee Employment Detail", emp.name, "work_shift")
	...
	att_doc = frappe.db.get_value("Attendance",{ "docstatus":("<",2),"employee": emp.name, "attendance_date":holiday}, ["name","status"], as_dict=1) or None
	print str(employee_start_time)
	if att_doc: att_time, att_status= att_doc.name, att_doc.status

	dep_doc = frappe.db.get_value("Departure", {"employee":emp.name,"departure_date":holiday ,"docstatus":("<", 2)}, ["name","status"], as_dict=1) or None
...
@frappe.whitelist(allow_guest=True)
def check_attendance_monthly():
	#date, hour
	#now = now_datetime()
	mydate = now_datetime() #get_datetime(date)
	...
	month_range= monthrange(cint(mydate.year), int(month))[1]
	for d in range(1,month_range+1):
		day = str(getdate(mydate).year)+'-'+str(month)+'-'+str(d)
		hour = mydate.hour
		print day
		if getdate(day) < getdate(mydate):
			check_attendance(day, hour)

def check_attendance(ddate=nowdate(),hour=20):
	...
	#print cur_time.hour
	for emp in employees:
		attendance = frappe.db.get_value("Attendance", {"employee":emp.name,"attendance_date":ddate,"docstatus":("<", 2)}, "name")
		departure = frappe.db.get_value("Departure", {"employee":emp.name,"departure_date":ddate,"docstatus":("<", 2)}, "name")

		emp_work_shift = frappe.db.get_value("Employee", emp, "work_shift")
		end_time = frappe.db.get_value("Work Shift Details", {"parent":emp_work_shift,"day":today}, "end_work")
		#print emp.name
		if cur_time.hour== hour and not attendance and not departure:
			print 'added'
			atten = frappe.new_doc('Attendance')
			atten.update({'name':emp.employee_name,'employee_name':emp.employee_name,'employee':emp.name,'status':'Absent',"docstatus":1,"attendance_date":ddate,"attendance_time":'00:00:00'})
			atten.flags.ignore_validate=True
			atten.save(ignore_permissions=True)

			dept = frappe.new_doc('Departure')
			dept.update({'name':emp.employee_name,'employee_name':emp.employee_name,'employee':emp.name,'status':'Absent',"docstatus":1,"departure_time":'00:00:00',"departure_date":ddate})
