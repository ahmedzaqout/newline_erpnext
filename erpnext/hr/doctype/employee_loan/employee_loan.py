...
class EmployeeLoan(AccountsController):
	def validate(self):
		repayment_amount_type= self.monthly_repayment_amount
		if self.daily_repayment_amount:
			repayment_amount_type= self.daily_repayment_amount
		elif self.yearly_repayment_amount:
			repayment_amount_type= self.yearly_repayment_amount

		check_repayment_method(self.repayment_method, self.loan_amount,repayment_amount_type, self.repayment_periods)
		if not self.company:
	...
	def make_repayment_schedule(self):
		...
		if self.company == "Nawa":
			if self.allowance_period and self.allowance_period > 0 :
				payment_date = frappe.utils.add_days(payment_date, self.allowance_period);
				

		while(balance_amount > 0):
			interest_amount = rounded(balance_amount * flt(self.rate_of_interest) / (12*100))
			principal_amount = ((self.monthly_repayment_amount or self.yearly_repayment_amount or self.daily_repayment_amount)) - interest_amount
			balance_amount = rounded(balance_amount + interest_amount - (self.monthly_repayment_amount or self.yearly_repayment_amount or self.daily_repayment_amount))
