...
class Departure(Document):
	...
	def validate_attendance_date(self):
		...
		if self.departure_date and ( isinstance(self.departure_date, str) or isinstance(self.departure_date, unicode)) :
			departure_date = datetime.datetime.strptime(self.departure_date , '%Y-%m-%d')
		att_time_next = frappe.db.get_value("Attendance",{ "docstatus":("<",2),"employee": self.employee, "attendance_date":(departure_date- timedelta(days=1))}, "attendance_time")
		...
		employee_start_time_next = frappe.db.get_value("Work Shift Details", {"parent":employee_work_shift,"day":day_next,"next_day":1}, "start_work")
		employee_end_time_next = frappe.db.get_value("Work Shift Details", {"parent":employee_work_shift,"day":day_next,"next_day":1}, "end_work")
		...
		if employee_end_time_next:
			att_from_time= frappe.db.sql('select TIMESTAMP(%s , %s)',(self.departure_date, employee_end_time_next))[0][0]
		...
		if employee_end_time_next:
			...
			from_time= frappe.db.sql('select TIMESTAMP(%s , %s)',(departure_date- timedelta(days=1), att_time))[0][0]
		else:
			to_time= frappe.db.sql('select TIMESTAMP(%s , %s)',(self.departure_date, self.departure_time))[0][0]
		...
		if holiday_list:
			...
			if is_holiday:
				if employee_start_time_next:
					total_att =datetime.timedelta(hours=24) - time_diff(str(att_time),str(self.departure_time) )
					shift_att = datetime.timedelta(hours=24) - time_diff(str(employee_start_time_next), str(employee_end_time))
			...
			if is_holiday and total_att > shift_att and diff*60 > float(overtime_max_minutes) :
				dep_time= att_time + shift_att	
				if employee_start_time_next:
					dep_timedate= frappe.db.sql('select TIMESTAMP(%s , %s)',((departure_date- timedelta(days=1)), dep_time))[0][0]
				...
				if att_over:
					...
					if  frappe.get_value('Timesheet Detail',{'parent':over_det.name,'activity_type':_('Auto Entry: Overtime Attendance')}):
				...
				self.add_overtime(att_time,att_over,to_time,att_from_time,employee_start_time,employee_end_time, employee_start_time_next)
	...
	def add_overtime(self,att_time,att_over,to_time,att_from_time,employee_start_time,employee_end_time,employee_start_time_next):
		diff=0
		overtime_max_minutes = frappe.db.get_value("HR Settings", None, "overtime_max_minutes")	
		if not overtime_max_minutes:
			frappe.throw(_("Enter the value of max overtime in HR settings"))
