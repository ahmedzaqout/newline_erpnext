...
erpnext.hr.AttendanceControlPanel = frappe.ui.form.Controller.extend({
	...
	get_departure_template:function() {
		if(!this.frm.doc.att_fr_date || !this.frm.doc.att_to_date) {
			frappe.msgprint(__("Attendance From Date and Attendance To Date is mandatory"));
			return;
		}
		window.location.href = repl(frappe.request.url +
			'?cmd=%(cmd)s&from_date=%(from_date)s&to_date=%(to_date)s', {
				cmd: "erpnext.hr.doctype.upload_attendance.upload_attendance.get_departure_template",
				from_date: this.frm.doc.att_fr_date,
				to_date: this.frm.doc.att_to_date,
			});
	},
	...
	show_upload: function() {
		...
		frappe.upload.make({
			...
			callback: function(attachment, r) { console.log(r)
	...
	show_dep_upload: function() {
		var me = this;
		var $wrapper = $(cur_frm.fields_dict.upload_departure_html.wrapper).empty();

		// upload
		frappe.upload.make({
			parent: $wrapper,
			args: {
				method: 'erpnext.hr.doctype.upload_attendance.upload_attendance.upload_departure'
			},
			no_socketio: true,
			sample_url: "e.g. http://example.com/somefile.csv",
			callback: function(attachment, r) { console.log(r)
				var $log_wrapper = $(cur_frm.fields_dict.import_departure_log.wrapper).empty();
