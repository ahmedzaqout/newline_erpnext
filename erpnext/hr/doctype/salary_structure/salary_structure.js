...
cur_frm.add_fetch('employee1', 'company', 'company');
...
frappe.ui.form.on('Salary Structure', {
	onload: function(frm) {
		...
		frm.set_query("employee1", "employees", function(doc) {
			return {
			...
			callback:function (r) {
				...
				for (var i=0; i< r.message.length; i++) {
					if (employees.indexOf(r.message[i].name) === -1) {
						...
						row.base = r.message[i].basic_salary; //frm.$emp_dialog.get_value('base');
						row.variable = frm.$emp_dialog.get_value('variable');
						row.day_salary = r.message[i].day_salary;
						row.hour_cost = r.message[i].hour_cost;
						row.overtime_hour_cost = r.message[i].over_hrs;
					get_earnings(r.message[i].name,frm);
					get_deductions(r.message[i].name,frm);
					}
				}
...
frappe.ui.form.on("Salary Structure Employee", "employee", function(frm, cdt, cdn){
	var d =locals[cdt][cdn];

	get_earnings(d.employee,frm);
	get_deductions(d.employee,frm)
})

var get_earnings = function(employee,frm) {
	...
	frappe.call({
		...
		callback:function (r) { 
			...
			for (var i=0; i< r.message.length; i++) {
				if (earnings.indexOf(r.message[i].name) === -1) {
...
var get_deductions = function(employee,frm) {
	...
	frappe.call({
			...
			if(r.message !== undefined){
				...
				for (var i=0; i< r.message.length; i++) {
					if (deductions.indexOf(r.message[i].name) === -1) {
...
frappe.ui.form.on('Salary Detail', {
	...
	//frappe.model.get_value('Salary Component', frm.doc.salary_component, ['amount','ratio'],function(r) {
