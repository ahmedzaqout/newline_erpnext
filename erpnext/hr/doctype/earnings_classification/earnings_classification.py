...
class EarningsClassification(Document):	
	...
	def update_pnw(self, designation, ratio, degree):
		employees = frappe.get_all("Employee Employment Detail", fields=["name","employee_name","designation"],filters={'status':'Active'})
		for emp in employees:
			sal_det = frappe.db.get_value("Employee Salary Detail",{'employee':emp.name},["name","grade","grade_category"],as_dict=1)
			if  sal_det and sal_det.grade == degree and emp.designation == designation: #if degree var:
				sal_doc = frappe.get_doc("Employee Salary Detail",{'employee':emp.name})
			...
			# update salary slip
			sal_slip = frappe.db.sql("select name from `tabSalary Slip` where employee= %s and docstatus!=2 and month(start_date)=12 and year(start_date)= 2018",(emp.name),as_dict=1)
