...
def update_att(user_id,atype,time):
	...
	if emp:
		if atype == 0: #att
			if not frappe.get_value('Attendance',{'employee':emp.name,'attendance_date':getdate(time),'status':'Present'},"name"):
				att = frappe.new_doc('Attendance')
				att.update({
					'employee':emp.name,
					'employee_name':emp.employee_name,
			...
			if not frappe.get_value('Departure',{'employee':emp.name,'departure_date':getdate(time),'status':'Present'},"name"):
				dept = frappe.new_doc('Departure')
				dept.update({
					'employee':emp.name,
					'employee_name':emp.employee_name,
