...
//console.log(r.message['unmarked']);
...
erpnext.EmployeeSelector = Class.extend({
	...
	make: function(frm, employee) {
		...
		$.each(employee, function(i, m) {
			...
			var employee_name = m.employee_name;
			var image = m.image;
			if(!employee_name) employee_name = m.employee;
			if(!image) image = "assets/newlinetheme2/images/placeholder-user.png";
//console.log(employee);
			$(repl('<div class="col-sm-3 unmarked-employee-checkbox">\
				<div class="checkbox">\
				<label><input type="checkbox" class="employee-check" employee="%(employee)s"/>\
				<div style="max-width:80px"><img src="%(img)s" class="img-responsive img-circle"/> %(employee)s </label></div>\
				</div></div>', {employee: employee_name ,img: image})).appendTo(row);
		});
