...
def get_employees(date, department = None, branch = None, company = None):
	...
	employee_list = frappe.get_list("Employee", fields=["employee", "employee_name","image"], filters=filters, order_by="employee_name")
	marked_employee = {}
...
def mark_employee_attendance(employee_list, status, date, leave_type=None, company=None):
	...
	ttime = get_time(now_datetime())
	if status == "Absent":
		ttime = '0:00:00'# datetime.strptime('00:00:00', '%H:%M:%S').time()
	frappe.msgprint(str(ttime))
	for employee in employee_list:
		attendance = frappe.new_doc("Attendance")
		attendance.employee = employee['employee']
		attendance.employee_name = employee['employee_name']
		attendance.attendance_date = date
		attendance.attendance_time = ttime
		...
		if status == "Absent":
			departure = frappe.new_doc("Departure")
			departure.employee = employee['employee']
			departure.employee_name = employee['employee_name']
			departure.departure_date = date
			departure.departure_time = ttime
