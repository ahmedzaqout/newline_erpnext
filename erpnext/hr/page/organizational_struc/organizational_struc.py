...
def get_tree_nodes():
	...
	parent=''
	headquarterdata= frappe.db.sql("""select  name ,director, director_name as title ,tree_id as id from tabHeadquarter  where docstatus < 2 and ifnull(parent_headquarter,'') = %s""",(parent), as_dict=1)

	childdata = []
	for i in range(len(headquarterdata)):
		mybranch= frappe.db.sql("""select name ,director, director_name as title ,tree_id as id from `tabBranch`  where docstatus < 2 and ifnull(`parent_branch`,'') = %s""",(headquarterdata[i].name), as_dict=1)
		if mybranch:
			childdata2=[]

			for j in range(len(mybranch)):
				mymanagement= frappe.db.sql("""select  name ,director, director_name as title ,tree_id as id from `tabManagement`  where docstatus < 2 and ifnull(`parent_management`,'') = %s""",(mybranch[j].name), as_dict=1)
				if mymanagement:
					childdata3=[]
					for c in range(len(mymanagement)):
						mycircle= frappe.db.sql("""select  name ,director, director_name as title ,tree_id as id from `tabCircle`  where docstatus < 2 and ifnull(`parent_circle`,'') = %s""",(mymanagement[c].name), as_dict=1)
						if mycircle:
							childdata4=[]
							for d in range(len(mycircle)):
								mydep= frappe.db.sql("""select  name ,director, director_name as title ,tree_id as id from `tabDepartment` where docstatus < 2 and ifnull(`parent_department`,'') = %s""",(mycircle[d].name), as_dict=1)
								if mydep:
									##
									childdata5=[]
									for sd in range(len(mydep)):
										#####
										subdep=frappe.db.sql("""select  name ,director, director_name as title ,tree_id as id from `tabSub Department` where docstatus < 2 and ifnull(`parent_sub_department`,'') = %s""",(mydep[sd].name), as_dict=1)
																						
										if subdep and levels == '6':
											childdata6=[]
											for se in range(len(subdep)):
												subbdep=frappe.db.sql("""select name ,director, director_name as title ,tree_id as id from `tabSub Association` where docstatus < 2 and ifnull(`parent_sub_association`,'') = %s""",(subdep[se].name), as_dict=1)
												childdata7=[]
												if subbdep and levels == '7':
													if levels == '7':
														childdata7= get_emp_children(subbdep)			
													
													data6={'name':subdep[se].name,'title':subdep[se].title,'id':subdep[se].id,'children':childdata7}
													childdata6.append(data6)
												else:
													if levels == '6':
														childdata6= get_emp_children(subdep)
													else:
														data6={'name':subdep[se].name,'title':subdep[se].title,'id':subdep[se].id}
														childdata6.append(data6)
												
											data5={'name':mydep[sd].name,'title':mydep[sd].title,'id':mydep[sd].id,'children':childdata6}
											...
											if levels == '5':
												...
												data5={'name':mydep[sd].name,'title':mydep[sd].title,'id':mydep[sd].id}
												childdata5.append(data5)
...
def get_emp_children(empqry):
	...
	for e in range(len(empqry)):
		emp=frappe.db.sql("""select  employee as name , employee_name as title ,0 as id from `tabResponsible Of Staff` where parent = %s""",(empqry[e].director), as_dict=1)
		if emp:
			data7={'name':empqry[e].name,'title':empqry[e].title,'id':empqry[e].id,'children':emp}
