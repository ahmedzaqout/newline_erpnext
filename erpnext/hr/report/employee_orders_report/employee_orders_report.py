...
def salaries(conditions, filters):
	...
	if filters.type=="Employee Edit Time":
		edit_time_orders= frappe.db.sql("""select em.employee_name , em.management, em.designation, em.circle, et.attendance_date, et.attendance_time,et.departure_time,et.docstatus from `tabEmployee Edit Time` et left join `tabEmployee` as em on et.employee=em.name  where  et.docstatus <2 %s order by et.attendance_date desc """ %
	...
	if filters.type=="Timesheet":
		timesheet = frappe.db.sql("""select em.name as employee ,em.employee_name , em.management, em.designation, em.circle,ts.end_date,ts.start_date,ts.type,ts.docstatus,ts.total_hours from `tabTimesheet` ts left join `tabEmployee` as em on ts.employee=em.name  where  ts.docstatus <2 %s order by ts.start_date desc""" %
		conditions, filters, as_dict=1 )
	...
	if filters.type=="Exit Permission":
		permission_orders = frappe.db.sql("""select em.name as employee ,em.employee_name , em.management, em.designation, ep.* from `tabExit permission` as ep  left join `tabEmployee` as em on ep.employee=em.name  where  ep.docstatus <2 %s order by ep.permission_date desc """ %
		conditions, filters, as_dict=1 )
	...
	if filters.type=="Leave Application":
		leave_orders = frappe.db.sql("""select em.name as employee ,em.employee_name , em.management, em.designation, la.* from `tabLeave Application` as la left join `tabEmployee` as em on la.employee=em.name where  la.docstatus <2 %s order by la.from_date desc """ %
		conditions, filters, as_dict=1 )
