...
def trial_employee_query(doctype, txt, searchfield, start, page_len, filters):
#	from frappe.desk.reportview import get_match_cond
	txt = "%{}%".format(txt)
	return frappe.db.sql("""select employee from `tabEmployee Employment Detail` where status='Active' and (scheduled_confirmation_date between %s and %s) and DATE_ADD(scheduled_confirmation_date,INTERVAL duration MONTH)>= CURDATE()""",(filters.get('from_date'),filters.get('to_date')) )
