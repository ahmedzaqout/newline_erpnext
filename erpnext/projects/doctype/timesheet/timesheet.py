...
class Timesheet(Document):
	...
	def validate_dates(self):
		for data in self.time_logs:
			...
			def_over = frappe.db.get_value("HR Settings", None, "max_working_hours_against_timesheet")
			if time_diff_in_hours(data.to_time, data.from_time) >= float(def_over) :
				frappe.throw(_("Overtime cannot be more than Max value"))


			if self.type!='compensatory' and is_overtime_exceeded(self.employee, data.from_time):		
		...
		if not employee_end_time:
			...
			diff_time = frappe.db.sql("select format(((TIME_TO_SEC('%s')-TIME_TO_SEC('%s'))),0)" %(str(employee_start_time), str( data.from_time)))[0][0]
			diff_time2 = frappe.db.sql("select format(((TIME_TO_SEC('%s')-TIME_TO_SEC('%s'))),0)" %(str(employee_end_time ), str(data.to_time)))[0][0]
			...
			if holiday_list:
				...
				for holiday in holidays:
					if holiday.holiday_date == getdate(data.from_time) or holiday.holiday_date == getdate(data.to_time):
							hold_f=True;
						#frappe.msgprint(str(data.to_time)+" n "+str(diff_time)+" f "+str(diff_time2))
			#if not hold_f:
			#	if  get_time(data.from_time) > get_time(employee_start_time) and get_time(data.from_time) < get_time(employee_end_time):
			#		frappe.throw(_("Overtime cannot be in attendance time"))
			#	if get_time(data.to_time) < get_time(employee_end_time) and get_time(data.to_time) > get_time(employee_start_time):
			#		frappe.throw(_("Overtime cannot be in attendance time"))
