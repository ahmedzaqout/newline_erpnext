...
def auto_create_fiscal_year():
	...
	for d in frappe.db.sql("""select name from `tabFiscal Year` where year_end_date = date_add(current_date, interval 3 day)"""):
		try:
			...
			frappe.msgprint(str(start_year if start_year==end_year else (start_year + "-" + end_year)))
			new_fy.insert(ignore_permissions=True)
			#Edited by Maysaa
			frappe.db.set_value("Global Defaults", None, "current_fiscal_year", start_year if start_year==end_year else (start_year + "-" + end_year))
			global_defaults = frappe.get_doc("Global Defaults")
			global_defaults.check_permission("write")
			global_defaults.on_update()
