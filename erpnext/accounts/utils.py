...
################################Edited by Maysaa###########################################
@frappe.whitelist()
def set_company(company,email):
	if  company :
		default_warehouse = frappe.db.get_value("Warehouse", filters={
			"warehouse_name": _("Stores"),
			"company": company
		})
		stock_settings = frappe.get_doc("Stock Settings")
		frappe.defaults.set_user_default("Company", company, email)
		frappe.defaults.set_global_default("company", company)
		# frappe.db.sql("update  tabDefaultValue set defvalue='{company}' where defkey='Company'").format(company=company)
		frappe.defaults.set_global_default("default_warehouse",default_warehouse)
