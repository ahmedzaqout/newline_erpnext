...
class SMSCenter(Document):
	def create_receiver_list(self):
		...
		if self.send_to in ['All Contact', 'All Customer Contact', 'All Supplier Contact', 'All Sales Partner Contact']:
			...
			rec = frappe.db.sql("""select epd.employee_name, phone_number from
				`tabEmployee Personal Detail` as epd join `tabEmployee Employment Detail` as ed on (`ed`.employee = `epd`.employee) where status = 'Active' and epd.docstatus < 2 and
				ifnull(phone_number,'')!='' %s""" % where_clause)
